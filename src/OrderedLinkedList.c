...
#include <stdio.h>
#include <stdlib.h>
#include <values.h> // To have MAXFLOAT
...
int main(int argc, char *argv[])
{
    ...
    rewind(datafile);
    // After rewinding the file, it allocates memory for the vertices and reads them into the 'vertexlist' array.
...
// The function Oenqueue inserts vertices into an ordered queue, keeping the elements sorted by the value field of the Vertex.
int Oenqueue(Vertex *vert, OQueue *Q)
{
    ...
    while(element_iter->next != NULL && element_iter->next->vertex->value < vert->value) element_iter = element_iter->next;
...
void printOqueue(OQueue Q)
{
    ...
    while(element_iter != NULL){
        printf("Vertex id = %s, value = %lf.\n",element_iter->vertex->id, element_iter->vertex->value);
